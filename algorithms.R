...
MILIMS4TS <- function(bags.list.list, bag_labels, instance_nums.list, 
  ...
  if(rho<=0){
    warning("The output model is probably worthless because the margin is 0. Try to set the larger nu.")
  }  
...
MILIMS <- function(bags.list, bag_labels, instance_nums, param.list, seed=1, SHAPELET=FALSE, restart_num=MAX_RESTART){
  ...
  for(iter in 1:T){
    ...
    if(optimizer==1){
      ...
      sol <- MI_WeakLearn_AdaGrad(Kpos.list, Kneg.list, 
                                  dpos, dneg, max_vals.list, shape_num, SHAPELET)      
    }
    alpha <- sol$alpha
    d_nonzero_id <- which(d!=0)
    Kall_vals <- rep(NA, m)
...
MI_WeakLearn <- function(neg_instance_nums,Kpos.list, Kneg.list, dpos, dneg, max_vals.list, shape_num, threshold, SHAPELET){
  ...
  bvec <- c(rep(0, nrow(Amat)-1), 1)
  dir <- c(rep("L", nrow(Amat)-1),"L")
  xopt_old <- 10000
  lb <- c(rep(0, (shape_num*2)), rep(-Inf, length(dneg_nonzero_id)))
  for(k in 1:MAX_WLT){
    temp_id <- 1
    ...
    if(xopt_old - solution$obj <= threshold){
      alpha <- matrix(solution$xopt[1:(shape_num*2)], nrow=1)
      break
    }else{
      xopt_old <- solution$obj
      alpha_old <- matrix(solution$xopt[1:(shape_num*2)], nrow=1)
    }
    if(k==MAX_WLT){
      alpha <- alpha_old
    }
  }
  return(list(alpha=alpha, optval = solution$obj))
}
...
run_LPBoost_dual <- function(x, y, nu){
  ...
  m <- ncol(x)
  obj <- c(rep(0, m),  1)
  mat1 <- cbind(t(as.numeric(y)*t(x)), matrix(-1, const_num, 1))
  mat2 <- c(rep(1, m), 0)
  mat <- rbind(mat1, mat2)
  rhs <- c(rep(0, const_num), 1)
  lb <- c(rep(0, m), -Inf)
  ub <- c(rep((1/nu), m), Inf)
  dir <- c(rep("L", const_num),"E")
  solution <- Rcplex(cvec=obj, Amat=mat, bvec=rhs, sense=dir, objsense = "min", lb=lb, ub=ub, control=list(trace=0, method=2))
  d <- solution$xopt[1:m]
  gamma <- solution$xopt[m+1]
  w <- solution$extra$lambda[1:const_num]
  return(list(d=d, gamma=gamma, w=w))
}
...
run_1normSVM <- function(x, y, nu){
  ...
  d <- ncol(x)
  m <- nrow(x)
  obj <- c(rep(0, d), 0, 1, rep(-1/nu, m))
  mat1 <- cbind(as.numeric(y)*x, matrix(y, m, 1), matrix(-1, m, 1), diag(1, m))
  mat2 <- c(rep(1,d), rep(0, m+2))
  mat <- rbind(mat1, mat2)
  rhs <- c(rep(0, m), 1)
  dir <- c(rep("G",m),"E")
  lb <- c(rep(0, d), -Inf, -Inf, rep(0,m))
  ub <- c(rep(1, d), Inf, Inf, rep(Inf,m))
  solution <- Rcplex(cvec=obj, Amat=mat, bvec=rhs, lb=lb, ub=ub, sense=dir, objsense = "max", control=list(trace=0,method=2))
  if(solution$status !=1){
    #    #message("cplex says objective function is not bounded, so try to solve hard margin problem")
    ub <- c(rep(1, d), Inf, 1, rep(Inf,m))
    solution <- Rcplex(cvec=obj, Amat=mat, bvec=rhs, lb=lb, ub=ub, sense=dir, objsense = "max", control=list(trace=0, method=2))
    if(solution$status !=1){
      #      #message("cplex says objective function is not bounded, so try to solve hard margin problem")
      ub <- c(rep(1, d), Inf, 1, rep(0,m))
      solution <- Rcplex(cvec=obj, Amat=mat, bvec=rhs, lb=lb, ub=ub, sense=dir, objsense = "max", control=list(trace=0, method=2))
    }
  }
  sol <- solution$xopt
